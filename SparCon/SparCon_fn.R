...
#Set color scheme
#orange magenta green blue - corr ctrl mut ko
color3 <- c(brewer.pal(11, "RdYlGn")[4],brewer.pal(8, "Accent")[c(1,5,6)])
sfm3 <- scale_fill_manual("Genotype", breaks=c("WT","HET","NULL", "CORR"), values = color3[c(4,1,2,3)])
scm3 <- scale_color_manual("Genotype", breaks=c("WT","HET","NULL", "CORR"), values = color3[c(4,1,2,3)])
...
mcc.icc.norm.well <- function(dat)
{
  ...
  ctrl.means <- mccd[ind_type=="CTRL" & (Setup == "EXP" | Setup == "MUTLAWN"),
                     lapply(.SD, geometric.mean),
                     by=.(exp_ID, wellID),
                     .SDcols = c("Syn_Number","Dend_Length")]
  
  print("Geometric means of controls from experimental wells:")
  print(ctrl.means)
  for (i in 1:dim(ctrl.means)[1])
  {
    iWell <- ctrl.means[i, wellID]
    print(paste("Normalized Well", iWell, sep = " "))
  ...
  print("Geometric means of GFP-labeled cells from color control wells:")
  print(ctrl.means2)
...
#function to normalize Sholl within-well
#will normalize to CTRL mean in each well by subtraction at each radius
#output: deltaCrossings
...
#plottin function for color controls
...
#plotring function for ECDF of color controls
...
#
# general dot-plot plotting function; expand/customize as necessary
...
#basic dot-plot function; allows to set several key variables in call
exp.plfn.geomPOINT <- function(gp, alpha = 0.5, size = 2.7, jitterwidth = 0.2, scale = 1, meanwidth = 0.3,
                               sewidth = 0.1)
{gp+
...
#third customization of geomPOINT, mostly for plotting
...
exp.plfn.dpSYN <- function(gp)
{gp+
    labs(y = "Normalized Synapse Number")+
    scale_y_continuous(limits = c(0.03,16), breaks = c(0.1, 2^(seq(0,8,1))))+
    coord_trans(y="log2")+
    theme(axis.title.x = element_blank(),
          axis.text.x = element_blank(),
          axis.title.y = element_blank(),
...
exp.plfn.dpDEND <- function(gp)
{gp+
    labs(x="Group", y = "Normalized Dendrite Length")+
...
exp.plfn.dpSEPSCFREQ <- function(gp)
{gp+
    labs(x="Group", y = "Normalized sEPSC Frequency")+
    scale_y_continuous(limits = c(0.01,256), breaks = c(2^(seq(0,9,2))), labels = fmt.round(1))+
    coord_trans(y="log2")+
    theme(axis.title.x = element_blank(),
          axis.text.x = element_blank(),
          axis.title.y = element_blank(),
...
exp.plfn.dpSEPSCAMPL <- function(gp)
{gp+
    labs(x="Group", y = "Normalized sEPSC Amplitude")+
    theme(axis.title.x = element_blank(),
          axis.text.x = element_blank(),
          axis.title.y = element_blank(),
          axis.text.y = element_text(size = 10),
...
{gp+
    ...
    stat_summary(fun.y = mean, geom = "point", shape = 21, color = "black", size = 3, alpha = 0.75)+
...
plot_ecdf_facets <- function(db, var_list, var_names, pl_names, scale_x_list, trans_x, comp_labels, sfm = sfm3, scm = scm3, 
                             round = 0, margin=0.05, size = 3, width = 9.5, height = 6)
{
...
#output dotplots by group
plot_dp <- function(db, var_list, lawn_groups, comp_groups, scale_y_list, trans_list, file_mod = "norm", width = 1.2, height = 2.5)
{
  for (k in seq_along(var_list))
  {
    clr_ctr <- 1
    for (l in seq_along(lawn_groups))
    {
      for (i in seq_along(comp_groups))
      {
        if (dim(db[Lawn == lawn_groups[l] & comparison_var == comp_groups[i]])[1] > 0)
        {
          pl.out <- ggplot(db[Lawn == lawn_groups[[l]] & comparison_var == comp_groups[i]], 
                           aes_string(x = "comparison_label", y = var_list[[k]], group = "SHANK2_gtype"))
          pl.out <- pl.out %>% exp.plfn.geomPOINT(size = 3) %>% exp.plfn.dp() +
            scale_y_list[[k]]+
            trans_list[[k]]+
            scale_fill_manual("Genotype", breaks=c("CORR","WT","HET","NULL"), values = color3[color_groups[[clr_ctr]]])
          if (grepl("R841X", lawn_groups[l])) {lawn_filename <- "MUTLAWN"} else(lawn_filename = "CTRL_LAWN")
          ggsave(paste0("dotplot_", var_list[[k]], comp_groups[i],"_", unique(db[,growth_condition]),
                        "_", lawn_filename,"_",file_mod, ".pdf"), device = "pdf", width = width,height = height)
          clr_ctr <- clr_ctr+1
        }
      }
    }
...
{
  comp_groups <- unique(db[,.(Lawn, comparison_var, growth_condition)])
  ...
  for (i in 1:dim(comp_groups)[1])
  {
    comp_group_var <- as.character(comp_groups[i]$comparison_var)
    growth_var <- comp_groups[i]$growth_condition
    lawn_var <- comp_groups[i]$Lawn
    
    for (j in seq_along(var_list))
    {
      ctrl <- as.numeric(unlist(db[comparison_var == comp_group_var & ind_type == "CTRL" & Lawn == lawn_var & growth_condition == growth_var, .SD, .SDcols = var_list[[j]] ]))
      mut <- as.numeric(unlist(db[comparison_var == comp_group_var & ind_type == "MUT" & Lawn == lawn_var & growth_condition == growth_var, .SD, .SDcols = var_list[[j]] ]))
...
ad_results.clrctrl <- function(db, var_list)
{
  comp_groups <- unique(db[,.(Lawn)])
  ...
  for (i in 1:dim(comp_groups)[1])
  {
    ...
    for (j in seq_along(var_list))
    {
      ctrl <- as.numeric(unlist(db[Color == "GFP" & Lawn == lawn_var, .SD, .SDcols = var_list[[j]] ]))
      mut <- as.numeric(unlist(db[Color == "mKO2" & Lawn == lawn_var, .SD, .SDcols = var_list[[j]] ]))
      test_result <- ad.test(ctrl,mut)
      row <- data.frame(CTRL_grp_name = "GFP",
                        MUT_grp_name = "mKO2",
                        Lawn = lawn_var,
                        CTRL_n = length(ctrl),
                        MUT_n = length(mut),
...
ttest_results <- function(db, var_list)
{
  ...
  comp_groups <- unique(db[,.(Lawn, comparison_var)])
...
#Set color scheme
#magenta green blue - ctrl mut ko
...
sparc_extract <- function(dat, subset = "EXP")
{
  ...
  if (length(subset) > 1){
      ...
      {
        subset2 <- paste(subset2, subset[i], sep = "|")
        print(subset2)
      }
    }
  dat[grep(subset2, Setup),]
}
